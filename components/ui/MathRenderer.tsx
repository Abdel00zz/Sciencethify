...
const ExerciseItem: React.FC<ExerciseItemProps> = ({ docId, exercise, index, isFirst, isLast }) => {
  ...
  // Re-typeset le titre quand il change ou après les interactions
  useEffect(() => {
    if (!exercise.title || !titleRef.current) return;
    
    const MathJax = (window as any).MathJax;
    if (MathJax && MathJax.typesetPromise) {
      // Attendre un peu pour que le DOM soit stable
      const timer = setTimeout(() => {
        MathJax.typesetPromise([titleRef.current]).catch((err: any) => {
          console.warn('MathJax title typeset error:', err);
        });
      }, 50);
      
      return () => clearTimeout(timer);
    }
  }, [exercise.title, isEditorOpen, isDeleteOpen, isActionsOpen]);
  ...
  const handleMoveUp = useCallback(() => {
    if (!isFirst) {
        ...
        // Forcer le re-typeset après le réordonnancement
        setTimeout(() => {
          const MathJax = (window as any).MathJax;
            ...
            <div className="flex justify-between items-center gap-4">
                <div className="flex items-center gap-3 flex-grow min-w-0">
                    ...
                    {exercise.title && (
                        <div 
                          ref={titleRef}
                          className="font-semibold text-sm text-indigo-600 dark:text-indigo-400 italic truncate"
